# Workflow: Add Task to Google Tasks

## Expected Input

[task description]

This skill creates a new task in Google Tasks.

## Argument Handling

### When Request Details Are Provided

If the user provided request details, intelligently parse them to extract task information.
...
After parsing the user's request text, if the **task title** is still unclear, use `AskUserQuestion` to ask: "What task would you like to add?"
...
If the user did not provide request details, use `AskUserQuestion` to gather information:
